...
function submitForm(e){e.preventDefault();const f=e.currentTarget,name=$('#name').value.trim(),url=$('#url').value.trim(),cur=Number($('#current').value),total=$('#total').value?Number($('#total').value):null;if(!name){return $('#formError').textContent='Informe o nome da obra.'}if(url){try{const u=new URL(url);if(!['http:','https:'].includes(u.protocol))throw 0}catch{return $('#formError').textContent='Informe uma URL válida começando por http:// ou https://'}}if(total&&cur>total&&!confirm('O progresso é maior que o total. Deseja salvar mesmo assim?'))return;const id=$('#workId').value;const work={id:id||crypto.randomUUID(),name,type:$('#type').value,status:$('#status').value,current:cur,total,categories:$$('#categoryChoices input:checked').map(i=>i.value),url,rating:$('#rating').value,startDate:$('#startDate').value,endDate:$('#endDate').value,notes:$('#notes').value.trim(),cover:editingCover};if(!work.endDate&&work.status==='Completo')work.endDate=new Date().toISOString().slice(0,10);if(id)state.works=state.works.map(w=>w.id===id?work:w);else state.works.unshift(work);save();$('#workDialog').close();toast(id?'Obra atualizada!':'Obra adicionada à biblioteca!')}
...
$('#workForm').addEventListener('submit',submitForm);$('#type').addEventListener('change',progressName);$('#coverInput').addEventListener('change',e=>{const file=e.target.files[0];if(!file)return;if(file.size>2.5*1024*1024){$('#formError').textContent='Escolha uma imagem de até 2,5 MB.';return}const r=new FileReader();r.onload=()=>{editingCover=r.result;$('#coverPreview').style.backgroundImage=`url('${editingCover}')`;$('#coverPreview').innerHTML='';$('#removeCover').hidden=false};r.readAsDataURL(file)});$('#removeCover').onclick=()=>{editingCover='';$('#coverPreview').style.backgroundImage='';$('#coverPreview').innerHTML='<span>＋</span><small>Adicionar capa</small>';$('#removeCover').hidden=true};$('#newCategory').addEventListener('keydown',e=>{if(e.key==='Enter'){e.preventDefault();const c=e.target.value.trim();if(c&&!state.categories.some(x=>x.toLowerCase()===c.toLowerCase())){state.categories.push(c);save();renderCategories($$('#categoryChoices input:checked').map(i=>i.value).concat(c));e.target.value=''}}});$('#clearFilters')?.addEventListener('click',()=>{$('#search').value='';$('#typeFilter').value='';$('#statusFilter').value='';$('#categoryFilter').value='';if($('#sortFilter'))$('#sortFilter').value='recent';activeQuick='';renderLibrary()});['search','typeFilter','statusFilter','categoryFilter','sortFilter'].forEach(id=>$('#'+id)?.addEventListener('input',renderLibrary));$('#categoryForm').addEventListener('submit',e=>{e.preventDefault();const v=$('#categoryName').value.trim();if(v&&!state.categories.some(x=>x.toLowerCase()===v.toLowerCase())){state.categories.push(v);save();$('#categoryName').value='';manageCategories()}});$('#importData').addEventListener('change',e=>importData(e.target.files[0]));window.mergeGlobalCategories=list=>{if(!Array.isArray(list)||!list.length)return;const merged=[...new Set([...state.categories,...list])];if(merged.length!==state.categories.length){state.categories=merged;save()}};
$('#today').textContent=new Intl.DateTimeFormat('pt-BR',{weekday:'long',day:'numeric',month:'long'}).format(new Date());render();
